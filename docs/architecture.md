...
- **The admin dashboard is read-only by design.** `/admin` (Part 11) reports what's true right now; it has no mutation path of its own. Creating/editing stations through a UI is Part 12, and a dedicated verification-approval workflow (bulk actions on `NEEDS_REVIEW` stations) is Part 13 — both deliberately out of this page's scope so it stays a fast, simple overview rather than growing into either of those.
