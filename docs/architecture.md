...
- **`GET /api/stations/nearby` is a plain distance-only lookup, deliberately separate from `GET /api/recommendations`.** Added for the Charging Calculator's "find my nearest station" (Part 08 addendum) — same browser-geolocation UX as `RecommendationTool.tsx`'s "Use my location", but no vehicle input and no compatibility/power/rating/verification scoring, just `station-service.ts`'s `getNearestStations()` sorting real-coordinate stations by `haversineDistanceKm`. A station with no confirmed `latitude`/`longitude` is excluded, never assigned a guessed distance — same rule as the map. Two queries by design: a lightweight `{id, latitude, longitude}` pass over every coordinate-bearing station to rank, then a second fetch of `stationListInclude`/`toStationListItem()` for only the top matches, so a full station include is never pulled for stations about to be discarded.
- **`Vehicle.fullRangeKm` (Part 08 addendum) powers an optional km estimate alongside the calculator's existing kWh/time estimate** — `calculateChargingEstimate()` (`src/services/charging-calculator.ts`) scales it linearly by battery % (current/added/target range), same simplifying assumption as the existing constant-power time estimate. Nullable and never backfilled from battery capacity alone (that would require inventing an efficiency figure with no real source) — only the five catalog vehicles' manufacturer-published, ARAI-certified figures are seeded (`prisma/seed-vehicles.ts`, sources in `docs/data-model.md §9`), and a visitor's custom/manual vehicle simply has no range estimate unless they enter their own vehicle's published figure in the calculator's optional "Full range (km)" field.
...
- **The public "Power" and "Availability" filters on `/stations` no longer offer an "Unknown" option, by the same kind of explicit product decision.** The 6th power bucket ("Unknown" = no recorded `powerKw`) was removed outright from `POWER_BUCKETS` (`src/lib/config/power-buckets.ts`) since it existed only for that filter; `AVAILABILITIES` in `StationFilterPanel.tsx` and the `availability` query filter's own accepted values (`src/lib/validation/station.ts`) both dropped `"UNKNOWN"` too. This only removes the ability to filter specifically for "unknown power"/"unknown availability" chargers — a charger's real `powerKw`/`availability` data, the Prisma `ChargerAvailability` enum, and the admin charger form's own `Availability` picker (`AdminChargerManager.tsx`, which legitimately still defaults a new charger to `UNKNOWN` until an admin records otherwise) are all untouched.
...
## 12. E Sakhi Marg Architecture (later addendum)

- **A new, separate page (`/marg`), not a replacement of `/stations`** — an explicit product decision, made because `/stations` (the "Find Chargers" search/filter list) is depended on by other parts of the app (recommendations, admin, several internal links) and serves a genuinely different job (browsing/filtering the full station list vs. planning one specific journey). Both are reachable from the same nav bar side by side; nothing about `/stations` changed.
- **Real routing, not a straight line** — `src/lib/geo/osrm.ts` calls OSRM's public demo routing server (`router.project-osrm.org`) for an actual road-following route (geometry + real driving distance), the same "never fabricate a location-shaped thing" rule the map/coordinate-backfill work has followed throughout this project, just applied to a route instead of a point. No API key needed, but it's a shared public demo instance, not meant for heavy production traffic — `fetchDrivingRoute()` is the one function to change to swap in a paid provider (Mapbox Directions, Google Routes, OpenRouteService, ...) later; nothing else in the app talks to a routing provider directly. `src/lib/geo/nominatim.ts` plays the same swappable role for place-name search (Starting Point/Destination), proxied server-side (`GET /api/marg/geocode`) since Nominatim's usage policy needs a real `User-Agent` header a browser script can't set for itself.
- **Checkpoint selection is a real algorithm over real data, not a curated list** — `planJourney()` (`src/services/marg-service.ts`) candidates every `ACTIVE` station with a confirmed coordinate and at least one real, non-deleted, non-`UNAVAILABLE` charger matching the requested connector + charging mode, projects each onto the OSRM route's own polyline (nearest-vertex approximation — see `nearestPointOnRoute()`) to get its distance-along-the-route and its detour off the route, discards anything more than 8 km off the route, then greedily picks stations nearest to evenly-spaced target points along the route (skipping candidates within 15 km of an already-chosen one). A route under ~60 km simply gets zero checkpoints — reported honestly as "no charging stop was needed," never a forced, unnecessary stop.
- **Checkpoint spacing (~60 km) is a placeholder default, not a battery claim.** `MargPlanInput`/`MargCheckpoint` (`src/types/marg.ts`) intentionally don't include any battery/range field yet — battery-aware reachability (current %, estimated range, minimum arrival %, "this checkpoint may not be reachable") is a deliberately deferred upgrade. When it's added, it becomes a checkpoint-selection *input* (replacing the fixed spacing default) and a *post-hoc reachability check* over the same checkpoint list this version already produces — not a redesign of the route/checkpoint architecture itself.
- **Checkpoint availability follows the exact same honesty rule as everywhere else in this app** — `summarizeAvailability()` reports a real `AVAILABLE`/`BUSY` when at least one matching charger has it on record, and otherwise `UNKNOWN` (rendered in the UI as "Status unavailable," per the original request's own wording), never inferred or defaulted to "available."
- **The map wrapper gained a second, parallel marker/route system, not a rewrite** — `MapProvider.tsx` (still the only file that imports Leaflet directly) added `margMarkers`/`margRoute` props alongside its existing `markers` prop; a journey's few stops are deliberately never mixed into the station-list `MarkerClusterGroup`. `FlyToTarget` gained an alternate `bounds` form (`map.flyToBounds`) so a planned journey's full extent fits in view on first render, instead of only centering on one point.
