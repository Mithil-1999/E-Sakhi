...
## 9. Part 08 Addendum — Reference Vehicle Catalog

`Vehicle` (§4) had no seed data through Part 07 — nothing in the original Excel workbook describes vehicles, only stations/chargers. The charging calculator (Part 08) is the first feature that needs real `Vehicle` rows to pick from, so it's also where a small catalog gets seeded: `prisma/seed-vehicles.ts`, called from `prisma/seed.ts`'s `main()`.
